...
![從 Facebook 社團挑選職缺貼文，用 slash command 觸發簡報生成](/assets/posts/claude-code-automated-social-media-pipeline/fb-job-post-example.png)
...
![Claude Code 驗證職缺資訊的實際輸出：逐項比對來源、標記無法獨立驗證的欄位](/assets/posts/claude-code-automated-social-media-pipeline/validation-example.png)
...
![簡報生成 pipeline：logo 抓取、背景範本、QR code、Marp 渲染、R2 上傳各階段的輸入輸出與工具](/assets/posts/claude-code-automated-social-media-pipeline/slide-pipeline.png)
...
![Claude Code 透過 Buffer MCP 直接呼叫 create_post，帶入文案、圖片 URL、排程時間](/assets/posts/claude-code-automated-social-media-pipeline/buffer-mcp-example.png)
...
![子貼文：AI 模擬面試練習與職缺深度分析，自動接在主貼文後面](/assets/posts/claude-code-automated-social-media-pipeline/subposts-combined.png)
